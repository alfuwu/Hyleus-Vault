...
The method in which the Throne uses to determine which abilities and memories are "less efficient" or "more efficient" is gruesome; upon the death of a host, instead of the [[soul]] being granted freedom from the [[Mortal Plane]], it is imprisoned within the Throne itself and gutted into being a hollow shell without personality, will, or physical form. The [[soul]] is then used for determining these more complex decisions, and grant the Throne something of a will of its own. This is done because the Throne, in its base form, possesses no intelligence. It requires these [[Soul|souls]] in order to function properly.
...
In addition to optimizing the host's body, mind, and [[soul]], the Bloodied Throne makes its host extraordinarily difficult to kill, storing the [[Vital Magic|lifeblood]] of all those whom were slain by its hosts and using it to regenerate the body of the host, which it can do with relative ease due to the connection it bears with the host's [[soul]].

## Origin
The Bloodied Throne originated in [[Pyx]], though it has moved locations many times, having been in [[Miulya]], the Yiyicos, and [[Drastevarde]] at different points in time. The only continent it has not graced with its presence is [[Anto]].
